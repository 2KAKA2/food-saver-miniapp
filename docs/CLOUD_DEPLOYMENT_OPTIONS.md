...
首发优先考虑腾讯云轻量应用服务器，部署现有 Docker Compose 单机方案。原因是当前项目规模适合轻量服务器，腾讯云同时提供备案、DNS、证书、可观测平台和可挂载的轻量对象存储，首期运维路径比较短。

如果已有阿里云实名认证账号或更熟悉阿里云，也可以使用阿里云轻量应用服务器；现有 Docker、Caddy、MySQL 和 Redis 方案不依赖特定厂商。
